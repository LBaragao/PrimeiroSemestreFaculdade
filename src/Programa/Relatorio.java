...
public class Relatorio extends JFrame {
	...
	public Relatorio() {
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"Nome", "Vacina", "Celular", "E-mail", "Pr\u00F3xima Aplica\u00E7\u00E3o"
			}
		) {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String sql = "select c.nome, b.nome, c.telefone1, c.email, "
						+ "date_add(primeiraaplicacao, INTERVAL b.periodoentredoses day) as segundaaplicacao from aplicacao_vacina a inner join tb_vacina b on "
						+ "a.id_vacina = b.id_vacina inner join tb_pessoa c on a.id_pessoa = c.id_pessoa where a.todasdosesaplicadas = \"Não\" and date_add(primeiraaplicacao, INTERVAL b.periodoentredoses day) >= ? "
						+ "and date_add(primeiraaplicacao, INTERVAL b.periodoentredoses day) <= ?";
				ConnectionFactory factory = new ConnectionFactory();
				try (Connection c = factory.obtemConexao()) {
					// 3: Pré compila o comando
					PreparedStatement ps = c.prepareStatement(sql);
					ps.setDate(1, dataInicial != null ? java.sql.Date.valueOf(dataInicial) : null);
					ps.setDate(2, dataFinal != null ? java.sql.Date.valueOf(dataFinal) : null);
					...
					while(rs.next()) {
						modelo.addRow(new Object[] {rs.getString("c.nome"), rs.getString("b.nome"), rs.getString("c.telefone1"), rs.getString("c.email"), rs.getDate("segundaaplicacao")});
					}
		...
		JLabel lblTituloLabel = new JLabel("Pacientes que v\u00E3o tomar a segunda dose entre:");
